...
path_files = "/ssd-scratch/aditya15007/Test/deviceMotion_walking_return"
with open(path_csv_map) as csv_f:
    ...
    for row in reader:
        if rownum != 0:
            if(row[11]!=''):
                file_id = int(row[11]) #for device Motion WalkingReturn
...
fileout = "deviceMotion_walking_return_" + str(myfunc.__name__.split(".")[-1]) + args.x_y_z_norm + ".csv"

f = open(fileout,'w')
for recordId, path_folder in map_recordids_to_filepaths.items():
    try:
        for file in os.listdir(path_folder):
            if re.match("^deviceMotion_walking_return.json.items", file):
                json_file = file
